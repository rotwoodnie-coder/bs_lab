...
同时，本模块在代码层面与以下基础设施协同工作：

- MySQL：`backend/src/infrastructure/mysql/mysql-client.ts`
- 对象存储：`backend/src/infrastructure/storage/s3-storage.ts`
- 预签名响应：`backend/src/lib/presign-response.ts`
- 权限控制：`backend/src/lib/auth/permission-guard.ts`
- 角色权限：`backend/src/lib/auth/role-permissions.ts`
...
- 数据库设计：`docs/bs_exp_data-database-design.md`
...
```text
标准实验库 -> 实验草稿 -> 草稿保存 -> 审核 -> 发布 -> 作业分发 -> 反馈统计
```
...
本章按仓库中与实验业务直接相关的真实文件逐项列出。为保证可读性，按“后端接口、后端服务、后端仓库、前端页面、前端 hooks、前端 utils、前端 types、前端 API”分类。
...
- `frontend/src/app/(dashboard)/teacher/experiment-editor/_components/StepListSidebar.tsx`
...
- `frontend/src/app/(dashboard)/teacher/experiment-editor/hooks/use-editor-engine.ts`
...
- `frontend/src/app/(dashboard)/teacher/experiment-editor/hooks/use-step-management.ts`
