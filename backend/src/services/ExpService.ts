  ...
  try {
    ...
    if (isUpdate) {
      await conn.query<ResultSetHeader>(
        `UPDATE exp_msg SET exp_name = ?, choose_type = ?, subject_id = ?, school_level_id = ?, grade_id = ?, difficulty_id = ?, exp_principle = ?, exp_caution = ?, exp_danger = ?, class_hour = ?, coursebook_id = ?, unit_id = ?, standard_exp_id = ?, create_user_id = COALESCE(create_user_id, ?), update_user_id = ?, update_time = NOW(), status = COALESCE(status, 't') WHERE exp_id = ? AND is_deleted = 0`,
        [expName, (input as any).chooseType ?? (input as any).choose_type ?? null, (input as any).subjectId ?? (input as any).subject_id ?? null, (input as any).schoolLevelId ?? (input as any).school_level_id ?? null, (input as any).gradeId ?? (input as any).grade_id ?? null, (input as any).difficultyId ?? (input as any).difficulty_id ?? null, (input as any).expPrinciple ?? (input as any).exp_principle ?? null, (input as any).expCaution ?? (input as any).exp_caution ?? null, (input as any).expDanger ?? (input as any).exp_danger ?? null, (input as any).classHour ?? (input as any).class_hour ?? null, (input as any).coursebookId ?? (input as any).coursebook_id ?? null, (input as any).unitId ?? (input as any).unit_id ?? null, (input as any).standardExpId ?? (input as any).standard_exp_id ?? null, ownerUserId, ownerUserId, expId],
      );
    } else {
      await conn.query<ResultSetHeader>(
