...
async function main() {
    ...
    if (dryRun)
        console.log(" 确认无误后去掉 --dry-run 执行");
    console.log("═══════════════════════════════════════════\n");
    ...
    try {
        // 1. 获取库中所有表
        const [tables] = await pool.query("SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA = ? ORDER BY TABLE_NAME", [dbName]);
        const allTables = tables.map((r) => r.TABLE_NAME.toLowerCase());
        ...
        for (const tbl of toClean) {
            const [typeRows] = await pool.query("SELECT TABLE_TYPE FROM information_schema.TABLES WHERE TABLE_SCHEMA = ? AND TABLE_NAME = ?", [dbName, tbl]);
            const tableType = String(typeRows[0]?.TABLE_TYPE ?? "");
